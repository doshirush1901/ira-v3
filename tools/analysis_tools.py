...
Security:
  ...
  - Rate/safety: TIMEOUT_SECONDS and MAX_OUTPUT_CHARS configurable via env or data/config/hephaestus_sandbox.json
"""
...
def _load_hephaestus_config() -> dict:
    """Load optional sandbox config (timeout_seconds, max_output_chars)."""
    path = PROJECT_ROOT / "data" / "config" / "hephaestus_sandbox.json"
    if path.exists():
        try:
...
_config = _load_hephaestus_config()
TIMEOUT_SECONDS = int(_config.get("timeout_seconds") or os.environ.get("HEPHAESTUS_TIMEOUT", "60"))
MAX_OUTPUT_CHARS = int(_config.get("max_output_chars") or os.environ.get("HEPHAESTUS_MAX_OUTPUT", "8000"))
...
def run_analysis(code: str, data: str = "") -> str:
    ...
    preamble_lines = [
        ...
        "",
        "# --- Sandbox restrictions: block filesystem and network access ---",
        "import socket as _socket",
        "class _BlockedSocket:",
        "    def __init__(self, *a, **k): raise PermissionError('Network access blocked in sandbox')",
        "_socket.socket = _BlockedSocket",
        "import pathlib as _pathlib",
        f"_SANDBOX_DIR = '{tmp_dir}'",
        "_orig_open = open",
        "def open(_path, *args, **kwargs):",
        "    _p = _pathlib.Path(str(_path)).resolve()",
        "    if not str(_p).startswith(_SANDBOX_DIR):",
        "        raise PermissionError(f'File access outside sandbox blocked: {_p}')",
        "    return _orig_open(str(_path), *args, **kwargs)",
