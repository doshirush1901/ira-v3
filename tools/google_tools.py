...
def create_calendar_event(
    ...
    Args:
        ...
        start_time: Start in ISO format or "YYYY-MM-DDTHH:MM:SS" (e.g. 2023-03-10T09:00:00-07:00).
        end_time: End in same format.
        attendees: Optional list of email addresses.
        timezone: IANA timezone (e.g. America/Los_Angeles). Used if start/end lack offset.
    ...
    try:
        ...
        try:
            ...
            log_event("MEETING_BOOKED", f"summary={summary} id={event_id}")
        except Exception:
            pass
        return event_id or "(created, no id)"
    except Exception as e:
...
def gmail_list_messages(query: str, max_results: int = 500) -> List[Dict[str, str]]:
    """List all message IDs and thread IDs matching the query (with pagination).

    Args:
        query: Gmail search query (e.g. "from:user@domain.com OR to:user@domain.com").
        max_results: Maximum total messages to return (default 500).

    Returns:
        List of {"id": message_id, "thread_id": thread_id} in reverse chronological order.
    """
    service = _get_gmail_service()
    if not service:
        return []
