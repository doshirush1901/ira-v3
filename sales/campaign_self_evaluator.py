    ...
    def _load_campaign_state(self) -> Dict:
        if CAMPAIGN_STATE_FILE.exists():
            try:
                ...
                logger.error(f"Error in _load_campaign_state: {e}", exc_info=True)
        return {}
    ...
    def _save_strategies(self, strategies: List[Strategy]):
        ...
        if STRATEGY_FILE.exists():
            try:
                ...
                logger.error(f"Error in _save_strategies: {e}", exc_info=True)
    ...
    def get_trend(self) -> Dict[str, Any]:
        ...
        if EVAL_HISTORY_FILE.exists():
            try:
                ...
                logger.error(f"Error in get_trend: {e}", exc_info=True)
