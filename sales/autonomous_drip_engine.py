...
ira@example-company.org on her own, tracks replies, scores herself,
...
Architecture:
    ...
    3. Send via Gmail API from ira@example-company.org
...
from openclaw.agents.ira.config import RUSHABH_TELEGRAM_ID
...
IRA_EMAIL = os.getenv("IRA_EMAIL", "ira@example-company.org")
RUSHABH_EMAIL = os.getenv("RUSHABH_EMAIL", "founder@example-company.org")
...
class AutonomousDripEngine:
    ...
    def _load_strategies(self) -> Dict[str, Any]:
        ...
        if STRATEGY_FILE.exists():
            try:
                ...
                logger.error(f"Error in _load_strategies: {e}", exc_info=True)
        return {
    ...
    def _load_dream_ideas(self) -> List[Dict]:
        ...
        if DREAM_IDEAS_FILE.exists():
            try:
                ...
                logger.error(f"Error in _load_dream_ideas: {e}", exc_info=True)
        return []
    ...
    def _load_batch_history(self) -> List[BatchResult]:
        ...
        if BATCH_HISTORY_FILE.exists():
            try:
                ...
                logger.error(f"Error in _load_batch_history: {e}", exc_info=True)
        return batches
    ...
    def run_daily_batch(self, max_emails: int = None, dry_run: bool = False) -> BatchResult:
        ...
        3. Send from ira@example-company.org
