...
class RealTimeObserver:
    ...
    def _get_api_key(self) -> str:
        if not self._api_key:
            self._api_key = os.environ.get("ANTHROPIC_API_KEY") or os.environ.get("OPENAI_API_KEY") or ""
            if not self._api_key:
                try:
                    from openclaw.agents.ira.config import ANTHROPIC_API_KEY, OPENAI_API_KEY
                    self._api_key = ANTHROPIC_API_KEY or OPENAI_API_KEY or ""
                except Exception:
                    pass
            ...
            if self._is_anthropic():
                ...
                response = await asyncio.wait_for(
                    client.messages.create(
                        ...
                        max_tokens=120,
                        system="Return only the extracted learning as plain text, or null.",
                ...
                for block in (response.content or []):
                    if getattr(block, "type", None) == "text":
                        learning = (getattr(block, "text", "") or "").strip()
                        break
            else:
                import openai
