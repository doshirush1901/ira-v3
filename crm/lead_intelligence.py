...
Usage:
    ...
    context = intel.get_lead_context("eu-012", "European-Lead-A")
...
class LeadContext:
    ...
    news_sentiment_reason: Optional[str] = None
...
class LeadIntelligence:
    ...
    def get_lead_context(
        ...
        """
        # Check cache first: by lead_id, then by company so multiple leads from same company reuse
        if not force_refresh:
            cached = self._load_cache(lead_id)
            if cached:
                return cached
            if company:
                company_key = self._cache_key_company(company)
                cached = self._load_cache(company_key)
                if cached:
                    cached.lead_id = lead_id
                    self._save_cache(lead_id, cached)
                    return cached
        ...
        if news:
            ...
            context.news_sentiment = news.get("sentiment")
            context.news_sentiment_reason = news.get("sentiment_reason")
        ...
        # Cache the result (by lead_id and by company for reuse)
        self._save_cache(lead_id, context)
        if company:
            self._save_cache(self._cache_key_company(company), context)
            ...
            if response.status_code == 200:
                ...
                for result in results.get("data", [])[:5]:
                    ...
                    if company_lower in title.lower() or company_lower in snippet.lower():
                        ...
                        if hook:
                            sentiment, reason = self._news_sentiment(title, snippet)
                            return {
                                "hook": hook,
                                "source": url,
...
Write one brief, specific sentence or "None"."""
        ...
        try:
            ...
            logger.error(f"Error in _llm_extract_insight: {e}", exc_info=True)
        
        return None

    def _news_sentiment(self, title: str, snippet: str) -> tuple:
        """Classify sentiment for B2B sales: positive/negative/neutral and one-line reason."""
        text = f"{title} {snippet}".lower()
        positive_words = ["expand", "expansion", "invest", "growth", "acqui", "launch", "new plant", "partnership", "profit", "revenue"]
        negative_words = ["layoff", "loss", "close", "decline", "cut", "downsize", "bankruptcy", "lawsuit"]
        pos = sum(1 for w in positive_words if w in text)
        neg = sum(1 for w in negative_words if w in text)
        if pos > neg:
            return ("positive", "News suggests positive development (expansion, investment, or growth).")
        if neg > pos:
            return ("negative", "News suggests negative development (layoffs, losses, or cuts).")
        if OPENAI_AVAILABLE:
            try:
                resp = client.chat.completions.create(
                    model="gpt-4.1-mini",
                    messages=[
                        {"role": "system", "content": "Classify sentiment for B2B sales: positive, negative, or neutral. One short reason."},
                ...
                if "positive" in content:
                    return ("positive", content[:200])
                if "negative" in content:
                    return ("negative", content[:200])
            except Exception:
                pass
        return ("neutral", "Sentiment unclear from headline/snippet.")

    def _cache_key_company(self, company: str) -> str:
        """Cache key by normalised company name so multiple leads from same company reuse intel."""
        norm = re.sub(r"[^a-z0-9]+", "_", (company or "").lower().strip())
        return f"company_{norm}_{hashlib.md5((company or '').encode()).hexdigest()[:8]}"
    ...
    def _load_cache(self, lead_id: str) -> Optional[LeadContext]:
        ...
        if cache_file.exists():
            try:
                ...
                logger.error(f"Error in _load_cache: {e}", exc_info=True)
...
def enrich_lead_for_email(
    ...
    if context.news_sentiment_reason:
        result["news_sentiment_reason"] = context.news_sentiment_reason
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Example: python lead_intelligence.py 'European-Lead-A' 'Germany' 'automotive,vehicle' 'https://example-lead.de'")
